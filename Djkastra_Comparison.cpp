...
public:
    ...
    vector<Node<double>*> Shortest_path_Fib(int source)
    {
        ...
        Fib_heap<double> pq;
        pq.create_heap(num_of_vertices,source+1);
        for(int i=0; i<num_of_vertices; i++)
        {
            bfs_visit_count[i]=0;
        }
        while(!pq.is_empty())
        {
            ...
            for(int i=0; i<dir_graph[temp].size(); i++)
            {
                ...
                Node<double> *x=pq.fib_heap_search(temp_dest+1);
                if(x!=NULL)
                {
                    if(x->value>z->value+temp_weight)
                        pq.fib_heap_decrease_key(temp_dest+1,z->value+temp_weight);
                    bfs_visit_count[temp_dest]=bfs_visit_count[temp]+1;
                }
            }
        }
