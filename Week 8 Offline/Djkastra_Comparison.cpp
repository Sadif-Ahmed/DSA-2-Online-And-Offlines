...
public:
    ...
    {
        Node<T> *x = createnode<T>(key,index); //Creating a new node
        found[index] = x;
        if(head==NULL)
            head = x;     //Creating head
        else
        {
            // Inserting the node to root list
            Node<T> *l1 = head->left;
            Node<T> *l2 = x->left;
            l1->right = x;
            x->left = l1;
    ...
    //Heap Creation For Djkastra Usage
    ...
    void consolidate()
    {
        ...
        Node<T>* A[max_deg+1];   //Temp array of respective rootlist degrees
        for(int i=0; i<=max_deg; i++)
            A[i] = NULL;
        Node<T> *w = head;
        unordered_map<Node<T>*,bool>mp;
        do
        {  //Combining roots of the same degree
            Node<T> *x = w;
            Node<T> *r = w->right;
            mp[w] = 1;
            int d = x->deg;
        ...
        head = NULL;
        for(int i=0; i<=max_deg; i++)
        {   //Final stage of fixing heap updating head
            if(A[i]!=NULL)
            {
                if(head==NULL)
                    head = A[i];
    ...
    // Extracting the head or minimum value
    Node<T>* fib_heap_extract_min()
    {
        ...
        Node<T> *z = head;
        found.erase(head->index);//Erasing from graph map
        if(z!=NULL)
        {
            // Adding previous head's children to root list
            Node<T> *ch = z->child;
            Node<T> *ch_copy = ch;
            do
            {
            ...
            while(ch!=ch_copy);
            // Removing previous head(z) from the root list
            ...
            {
                head = z->right;
                consolidate(); //Consolidation or fixing up the whole heap
            }
            num_of_nodes--;
        }
        if(z!=NULL)
...
private:
    ...
    void tickle_down(int index)
    {
        ...
        {
            smaller=(right(index).second<=left(index).second)?2*index+2:2*index+1;
            if(temp.second<=holder[smaller].second)
                break;
            holder[index]=holder[smaller];
            index=smaller;
        }
        holder[index]=temp;
    }
public :
    ...
    Bin_heap()
    {
        holder= new pair<int,T> [10000000];
        max_size=10000000;
        curr_size=0;
    }
    //Destructor
...
public:
    ...
    vector<dest_weight> *dir_graph_indexed;
    ...
    Graph(int num_v,int num_e)
    {
        ...
        dir_graph_indexed= new vector<dest_weight>[num_of_vertices+1];
    ...
    void Add_Edge_Directed(int startv,int endv,double weight=0)
    {
        ...
        dir_graph_indexed[startv+1].push_back(make_pair(endv+1,weight));


    }
    void Add_Edge_Undirected(int startv,int endv,double weight=0)
    {
        Add_Edge_Directed(startv,endv,weight);
        Add_Edge_Directed(endv,startv,weight);
    ...
    vector<Node<double>*> Shortest_path_Fib(int source)
    {
        ...
        while(!pq.is_empty())
        {
            ...
            result.push_back(z);
            int temp=(z->index);
            for(int i=0; i<dir_graph_indexed[temp].size(); i++)
            {
                int temp_dest=dir_graph_indexed[temp][i].first;
                double temp_weight=dir_graph_indexed[temp][i].second;
                ...
                if(x!=NULL)
                {
                    ...
                    bfs_visit_count[temp_dest-1]=bfs_visit_count[temp-1]+1;
                }
            }
        }
        return result;
    ...
    {   freopen("opt.txt","w",stdout);
        ...
        {    reset();
            ...
            auto dur_bin=duration_cast<nanoseconds>(end_bin-start_bin);
           ...
           // clock_t start_fib = clock();
            ...
            //vector<Node<double>*> res = Shortest_path_Fib(source[i]+1);
            vector<Node<double>*> res = Shortest_path_Fib(1);
            //result=double(clock() - start_fib) / CLOCKS_PER_SEC;
            auto end_fib=high_resolution_clock::now();
            auto dur_fib=duration_cast<nanoseconds>(end_fib-start_fib);
            ...
            for(int j=0; j<res.size(); j++)
            {
                ...
                if(z->index==(dest[i]+1))
                {
                   cout<<"Fibonacci Heap"<<endl;
                    cout<<"Vertex: "<<z->index-1<<"  Shortest Path cost  "<<z->value<<endl;
                    cout<<"Path Length  "<<bfs_visit_count[z->index-1]<<endl;
